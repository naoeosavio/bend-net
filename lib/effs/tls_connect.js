...
function tls_connect_raw(ip, sni, port, verify, alpn, k) {
  const sys = io_sys();
  ...
  const start_tls = () => {
    ...
    try {
      ...
      // ALPN mode: alpn=1 offers h2 (HTTPS); alpn=0 offers nothing
      // and accepts no negotiation (WSS/HTTP/1.1 Upgrade).
      if (alpn) {
        const offer = new Uint8Array([2, 104, 50]);
        lib.SSL_CTX_set_alpn_protos(ctx, lib.ptr(offer), 3);
      }
      ses = lib.SSL_new(ctx);
      if (!ses) {
    ...
    const step = () => {
      ...
      if (r === 1) {
        // ALPN: with alpn=1 the client offers only h2 (set above), so
        // OpenSSL itself aborts when the server selects anything else:
        // r === 1 already proves h2. With alpn=0 no offer is made and
        // no protocol is required (WSS rides HTTP/1.1 after TLS).
        let ok_verify = true;
        try {
