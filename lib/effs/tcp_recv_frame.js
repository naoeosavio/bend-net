...
function recv_frame_need() {
  // No read claim: the scheduler must run the body first so a frame
  // already complete in buf answers synchronously. Parking on the fd
  // before running would hang buf-completable reads until new socket
  // data (or FIN) arrives; the body parks itself on EAGAIN below when
  // it really needs more bytes.
  // return { read: true };
  return {};
}
